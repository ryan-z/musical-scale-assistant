...
HowTo.prototype.intentHandlers = {
  GetScale: (intent, session, response) => {
    ...
    if (scale) {
      ...
      response.tellWithCard(speechOutput, cardTitle, scale);
    } else {
      let speech = '';
...
var APP_ID = "amzn1.ask.skill.3dadf9a2-ce7e-4ca5-a8ca-5ab7809a3967";
...
HowTo.prototype.eventHandlers.onLaunch = function (launchRequest, session, response) {
    var speechText = "Welcome to Scale Helper. Just name a scale, such as G Melodic Minor. Now, which scale do you want notes for?";
    // If the user either does not reply to the welcome message or says something that is not
    // understood, they will be prompted again with this text.
...
HowTo.prototype.intentHandlers = {
    "GetScale": function (intent, session, response) {
        var scaleSlot = intent.slots.Scale,
            scaleName;
        if (scaleSlot && scaleSlot.value){
            scaleName = scaleSlot.value.toLowerCase();
        }
        var patternSlot = intent.slots.Pattern,
            patternName;
        if (patternSlot && patternSlot.value){
            patternName = patternSlot.value.toLowerCase();
        }
        var lookup = scaleName + ' ' + patternName;
        var cardTitle = "Notes for " + lookup,
            scale = scales[lookup],
            speechOutput,
            repromptOutput;
        if (scale) {
    ...
    "AMAZON.HelpIntent": function (intent, session, response) {
        var speechText = "Supported scales are major, natural minor, melodic minor, and harmonic minor.\
        For example, just say F melodic minor, or, you can say exit... Now, what can I help you with?";
        var repromptText = "If you want to hear the options again, just say Help.";
        var speechOutput = {
            speech: speechText,
            type: AlexaSkill.speechOutputType.PLAIN_TEXT
        };
